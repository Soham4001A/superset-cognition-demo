...
## Demo images in this fork

This fork carries an extra, non-published build file used only by the compliance-gate
demonstration:

- `demo.live.Dockerfile`: builds a minimal live demo service. It is **not** part of the
  official image matrix, is never pushed to Docker Hub, and is not supported for production
  use.
...
- `SENTINEL_LIVE_DEMO_FLAG`: set to `1` in `demo.live.Dockerfile` to mark a container as the
  live demo instance. It has no effect on Superset itself and should stay unset in every
  non-demo image.
