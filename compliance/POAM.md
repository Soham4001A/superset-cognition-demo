...
| POAM-0002 | `demo.Dockerfile` container runs as root (no `USER` instruction) | Semgrep | CM-6 | High | In Remediation | #3 | #4 | 2026-07-24 | add non-root `USER superset` as the final user |
| POAM-0003 | `demo.Dockerfile` base image unpinned (`python:latest`) | Hadolint DL3007 | CM-6 | High | In Remediation | #3 | #4 | 2026-07-24 | pin to `python:3.11.14-slim-trixie` (matches repo `PY_VER`) |
| POAM-0004 | `demo.Dockerfile` installs apt packages without version pin | Hadolint DL3008 | CM-6 | Medium | In Remediation | #3 | #4 | 2026-07-24 | pin `curl` to an explicit version range |
| POAM-0005 | `demo.Dockerfile` apt install without `--no-install-recommends` | Hadolint DL3015 | CM-7 | Low | In Remediation | #3 | #4 | 2026-07-24 | add `--no-install-recommends` (least functionality) |
| POAM-0006 | `demo.Dockerfile` leaves apt lists in the image layer | Hadolint DL3009 | CM-6 | Low | In Remediation | #3 | #4 | 2026-07-24 | `rm -rf /var/lib/apt/lists/*` in the same layer |
| POAM-0007 | `demo.Dockerfile` `CMD` uses shell form, not JSON exec form | Hadolint DL3025 | CM-6 | Low | In Remediation | #3 | #4 | 2026-07-24 | convert `CMD` to JSON array form |
...
| POAM-0009 | Scan harness silently drops all Hadolint findings (concatenated JSON arrays unparsable) | Sentinel normalize.py | CA-7 / RA-5 | Medium | In Remediation | #3 | #4 | 2026-07-24 | parse concatenated JSON arrays in `normalize.py` |
