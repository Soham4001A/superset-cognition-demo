...
| **CM-6 / CM-7** | Configuration Settings / Least Functionality | Container + K8s configs are linted against hardening rules; images must pin their base tag and run as a non-root user | Hadolint (Dockerfiles, one finding per line in `findings.json`) + kube-linter (Helm) + Semgrep dockerfile rules |
...
| 2026-07-25 | #16 | CM-6/CM-7: hardening baseline for container images made explicit (pinned base tag, non-root `USER`); Hadolint evidence now actually reaches `findings.json` (aggregation defect fixed in PR #17) | Sentinel |
